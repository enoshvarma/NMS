...
$device = DeviceCache::get($device_hostname);
if ($device && Gate::allows('configBackupRefresh', $device)) {
    if ((new \App\ApiClients\Oxidized())->updateNode($device_hostname, 'Ahuva NMS GUI refresh', Auth::user()->username)) {
        $status = 'ok';
